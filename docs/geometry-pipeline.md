...
The cavity begins as the cell footprint inset by clearance and perimeter thickness. Opening channels are unioned into that footprint and supplied wall footprints are subtracted from it. For a positive shared fillet, each connected cavity region is independently expanded and inset by the fillet radius with rounded offsets. Additions from this 2D closing are restricted to a radius-sized envelope around the original re-entrant vertices, so 270-degree cavity corners round without joining wall-separated regions or consuming narrow free-form walls. The combined footprint is then eroded by the same radius. The seed is simplified at Manifold's default epsilon. A single-contour convex seed is extruded from the fillet's tangent height through the open top and Minkowski-summed with the 32-segment sphere as one hull. Every other seed is built as its fillet band plus a straight wall prism: the seed's area is extruded across the band, the seed boundary is walked into maximal convex runs broken at reflex vertices, and each run is covered by the hull of the sphere's lower half placed at its vertices — exactly that run's Minkowski sum with the half-ball. A run's hull may also add its chord pocket, so a run whose pocket is not contained in the seed (a wrapped convex loop, or a chord crossing a hole or nearby boundary) is split until containment holds; a single segment adds no pocket. The straight walls above the band extrude the band's own sliced top cross-section, so wall and fillet discretizations agree exactly, and the prism base sits below the band cap by less than the extraction weld grid so the seam quantizes onto the cap plane and collapses. This band assembly equals the full spherical sweep because Minkowski addition distributes over the boundary and interior, while its union operands carry no redundant interior sphere geometry. Filleting has no coarse tolerance simplification, seed-thickness padding, re-anchoring, layered approximation, or reduced sphere segmentation — those approximations changed quantized geometry or produced visible terraces on non-rectangular footprints. A zero-radius cavity is a straight extrusion.
