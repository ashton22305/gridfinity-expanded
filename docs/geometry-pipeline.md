...
`generateGeometry()` creates one canonical Gridfinity base and translates it to every cell. Configuration-independent solids — the canonical base and each fillet radius's sphere and half-ball — are cached per WASM instance for the worker's lifetime, so repeat generations skip rebuilding them; cached solids are never deleted. Each bin's unioned cell footprint is constructed once and reused to derive both the outer body and cavity, avoiding duplicate 2D unions without changing either contour. It unions the translated bases with the complete outer-footprint extrusion. Positive outer contours use the fixed 3.75 mm Gridfinity radius for both convex corners and localized closing of re-entrant perimeter corners, so non-rectangular exterior wall junctions follow the same rounded profile without changing enclosed-hole contours or filling larger cutouts.

The cavity begins as the cell footprint inset by clearance and perimeter thickness. Opening channels are unioned into that footprint and supplied wall footprints are subtracted from it. For a positive shared fillet, each connected cavity region is independently expanded and inset by the fillet radius with rounded offsets. Additions from this 2D closing are restricted to a radius-sized envelope around the original re-entrant vertices, so 270-degree cavity corners round without joining wall-separated regions or consuming narrow free-form walls. The combined footprint is then eroded by the same radius. The seed is simplified at Manifold's default epsilon and decomposed into connected regions, and each region's cavity is constructed as one closed triangle mesh with no booleans: the region's wall polygon (the seed offset outward by the fillet radius with round joins) has every vertex exactly the fillet radius from the region, and nearest-point projection fibers between wall and region never cross, so latitude rings interpolated along those fibers as q + (w − q)·sin φ trace the exact spherical sweep surface — sphere corners emerge where fibers fan around a region vertex, ring zero closes fans by shared indices, straight walls extrude above the tangent height, and the floor and top caps are triangulated with `triangulate`. A single seamless mesh gives the final subtract only transversal intersections, which is what keeps the fillet free of weld-scale floor terraces; the earlier hull- and torus-assembled bands failed exactly there, because near-tangent seams between differently discretized pieces amplified float32 vertex noise past the weld grid. Concave trims may skip a few chord lengths of projection, which reproduces the sweep within the arc's chord sagitta; a projection jump beyond a radius-scaled threshold (a waist narrower than the sweep diameter) or a rejected mesh falls that region back to the exact Minkowski sweep with the cached 32-segment sphere. Filleting has no reduced segmentation and no coarse tolerance simplification, and a zero-radius cavity is a straight extrusion. Filleting has no coarse tolerance simplification, seed-thickness padding, re-anchoring, layered approximation, or reduced sphere segmentation — those approximations changed quantized geometry or produced visible terraces on non-rectangular footprints. A zero-radius cavity is a straight extrusion.
...
The hook derives `BinParameters[]` and increments a revision before asynchronously checking the per-bin IndexedDB cache. Edits start generating immediately when no generation is in flight — stale replies are discarded by revision — and coalesce behind a short debounce while one is, so bursts cannot queue uncancellable WASM work behind the pool. While a spatial edit gesture is held (the store's `gestureActive`, set by the Shape tab's paint gesture), the hook only invalidates stale work and starts nothing; releasing the gesture re-runs the effect and generates the final shape once. Cache keys are SHA-256 hashes over an explicit cache-version salt and every worker-consumed parameter except `binId`; editor identity and printer metadata therefore do not create duplicate meshes. Complete hits bypass worker messages. Partial hits post only missing bins, one single-bin message round-robin across a small worker pool (sized from `navigator.hardwareConcurrency`, capped at 4), then merge cached and generated bins back into design order. Successful worker results are persisted without delaying rendering.
