...
The cavity begins as the cell footprint inset by clearance and perimeter thickness. Opening channels are unioned into that footprint and supplied wall footprints are subtracted from it. For a positive shared fillet, each connected cavity region is independently expanded and inset by the fillet radius with rounded offsets. Additions from this 2D closing are restricted to a radius-sized envelope around the original re-entrant vertices, so 270-degree cavity corners round without joining wall-separated regions or consuming narrow free-form walls. The combined footprint is then eroded by the same radius. The complete seed, including disconnected regions and holes, is triangulated and greedily merged across shared edges only when the result remains convex. Complex single-contour seeds are simplified at Manifold's default epsilon, then every convex partition is extruded exactly from the fillet's tangent height through the open top, Minkowski-summed with the same 32-segment sphere, and the swept solids are unioned. This distributed sweep is mathematically identical to sweeping the complete seed because Minkowski addition distributes over union. Small seeds, disconnected regions, and holes retain the complete sweep when decomposition overhead or independently quantized sweep boundaries would be counterproductive. Only the lower hemisphere intersects the bin, producing rounded wall junctions, the rounded floor transition, and straight upper cavity as one exact solid. Filleting has no coarse tolerance simplification, seed-thickness padding, re-anchoring, layered approximation, or reduced sphere segmentation — those approximations changed quantized geometry or produced visible terraces on non-rectangular footprints. A zero-radius cavity is a straight extrusion.
