...
`generateGeometry()` creates one canonical Gridfinity base and translates it to every cell. Configuration-independent solids — the canonical base and each fillet radius's sphere and half-ball — are cached per WASM instance for the worker's lifetime, so repeat generations skip rebuilding them; cached solids are never deleted. Each worker also owns two four-entry evaluated LRU caches in its WASM instance: finished bin solids keyed by every ordered geometry parameter except `binId` and `pieces`, and non-convex fillet bands keyed by the simplified seed polygons and radius. A hit is refreshed, insertion forces evaluation with `numVert()`, normal callers never delete cached values, and eviction explicitly deletes the least-recently-used manifold. Finished solids intentionally exclude piece footprints so cut edits reuse construction and repeat only piece intersection; band reuse also survives height edits because the lower fillet band does not depend on wall height. Convex spherical sweeps remain uncached. Each bin's unioned cell footprint is constructed once and reused to derive both the outer body and cavity, avoiding duplicate 2D unions without changing either contour. It unions the translated bases with the complete outer-footprint extrusion. Positive outer contours use the fixed 3.75 mm Gridfinity radius for both convex corners and localized closing of re-entrant perimeter corners, so non-rectangular exterior wall junctions follow the same rounded profile without changing enclosed-hole contours or filling larger cutouts.

The cavity begins as the cell footprint inset by clearance and perimeter thickness. Opening channels are unioned into that footprint and supplied wall footprints are subtracted from it. For a positive shared fillet, each connected cavity region is independently expanded and inset by the fillet radius with 32-segment rounded offsets. Additions from this 2D closing are restricted to a radius-sized envelope around the original re-entrant vertices, so 270-degree cavity corners round without joining wall-separated regions or consuming narrow free-form walls. The combined footprint is then eroded by the same radius. The seed is simplified at Manifold's default epsilon. A single-contour convex seed is extruded from the fillet's tangent height through the open top and Minkowski-summed with a 16-segment cavity sphere as one hull. Every other seed is built as its fillet band plus a straight wall prism: the seed's area is extruded across the band, the seed boundary is walked into maximal convex runs broken at reflex vertices, and each run is covered by the hull of the 16-segment sphere's lower half placed at its vertices — exactly that run's Minkowski sum with the half-ball. A run's hull may also add its chord pocket, so a run whose pocket is not contained in the seed (a wrapped convex loop, or a chord crossing a hole or nearby boundary) is split until containment holds; a single segment adds no pocket. The straight walls above the band extrude the band's own sliced top cross-section, so wall and fillet discretizations agree exactly, and the prism base sits below the band cap by less than the extraction weld grid so the seam quantizes onto the cap plane and collapses. This band assembly equals the full spherical sweep because Minkowski addition distributes over the boundary and interior, while its union operands carry no redundant interior sphere geometry. Sixteen segments retain a smooth printable cavity transition while materially reducing the dominant spherical-sweep cost; the 3.75 mm exterior profile and every 2D rounded offset remain at 32 segments. There is no coarse tolerance simplification, seed-thickness padding, re-anchoring, or layered approximation. A zero-radius cavity is a straight extrusion.

`generateGeometry()` is asynchronous and accepts an optional internal band-union delegate. Without one, convex cavities and bands with fewer than 20 exact boundary chains retain the serial construction above. For a larger non-convex band, the delegate sorts chains by centroid X and divides contiguous groups across the primary plus currently idle helper workers. Helper groups are posted first, the primary unions one group locally while they run, and returned group meshes are reconstructed in the primary WASM instance before one union with the seed core. Mesh handoff preserves `numProp`, float32 vertex properties, triangle indices, merge vectors, and tolerance through transferables. The completed evaluated band enters the same four-entry band LRU as serial construction; the final band/prism seam, solid booleans, piece intersection, and `manifoldTriangles()` extraction boundary are unchanged.
...
The hook derives `BinParameters[]`, debounces changes, and increments a revision before asynchronously checking the per-bin IndexedDB cache. Cache keys are SHA-256 hashes over an explicit cache-version salt and every worker-consumed parameter except `binId`; editor identity and printer metadata therefore do not create duplicate meshes. Complete hits bypass worker messages. Partial hits queue only missing bins as single-bin tasks in a small worker pool (sized from `navigator.hardwareConcurrency`, capped at 4), then merge cached and generated bins back into design order. FNV-1a hashing of the stable `binId` selects a home worker, keeping that worker's WASM-local intermediate LRUs useful across edits even when bin ordering changes. Home-bin work has priority: a band may borrow only workers that are idle and have no queued home task, and helper count is capped at `poolSize - activeBins`. One-worker pools and small bands stay serial.

Generation, helper requests/results, completion/failure, and cancellation use an internal discriminated worker protocol; all payloads remain structured-clone compatible. A helper error or 10-second timeout replaces only that helper slot and makes the primary regenerate the complete band serially. New revisions cancel primaries awaiting helper work, discard late replies, and never surface stale work as an application error. Scheduling, worker occupancy, helper coordination, and revision state all live in refs, so bookkeeping does not cause React renders. Successful worker results are persisted without delaying rendering.

Cache records contain structured-cloned per-piece `Float32Array` triangle soups and footprint cells, not STL bytes or preview transforms. The `geometry-cache-v2` salt invalidates meshes generated with the former cavity-sphere resolution. Reads validate the record shape and return hits independently of a best-effort access-time refresh. Writes trigger cursor-based least-recently-used eviction when the approximate stored mesh size exceeds 100 MB, scanning one record at a time rather than materializing the full cache. Failed database opens are retried on later operations. Hashing, IndexedDB, corruption, and quota errors are all treated as cache misses or ignored writes; only worker-generation failures become the hook's generic geometry error. Revision checks discard stale cache lookups and worker replies, and each completed `Bin[]` remains paired with the design snapshot that produced it.

## Printability gates

`npm run check:manifold` exercises valid rectangular, irregular, U, ring/hole, opening, concave-perimeter, a 21-cell diagonal staircase, T- and cross-wall-junction, hardware, multiple-bin, multipart, and minimum-height/maximum-fillet fixtures through the production `buildBinParameters → generateGeometry` path. It reconstructs topology from triangle coordinates and requires non-empty pieces, watertight edges, consistent winding, no degenerates, duplicate faces, membranes, serialized-STL topology errors, or near-horizontal faces (by face normal) inside the fillet transition band — the terrace signature. Full-height wall fixtures must also retain interior top faces, guarding against a closing operation consuming narrow walls. The gate also asserts the mirrored parameter-boundary orientation and the 0.3 mm multipart gap through the viewer-branch `previewLayout()`.
