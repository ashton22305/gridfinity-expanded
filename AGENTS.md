...
There is no dedicated unit test runner configured. For non-trivial changes, run `npm run build` and `npm run lint`. For geometry, split, STL export, printer-fit, or config-shape changes, also run `npm run check:manifold`.
...
Tabs should read and write through `useAppStore()` instead of prop drilling. In auto split mode, `store.ts` re-derives `splitLines` on every config or printer write, so use existing setters and keep `BinConfig` plain JSON-compatible data for worker `postMessage`.
...
The outer wall always follows the Gridfinity spec profile: 41.5 mm top width and 3.75 mm outer radius. `cavityCornerRadius` affects only the cavity interior. `GridCell.bin` creates separate logical bins; adjacent cells with different bin ids become separate complete bins with their own walls.

`openEdges` remove perimeter walls, and `dividerEdges` add internal grid-aligned walls. Free-form `innerWalls` are mm segments with per-wall width and optional height; lower walls ramp into taller structures in the manifold path. `baseSlopes` are per logical bin, and zero-angle slope entries should not be persisted. Split seam edges are open unless a divider lies exactly on the split line. Every output mesh (previews and export pieces) is mirrored across Y on the way out of `generateBinPieces`/`generateBinPiecesJscad` because the editors map SVG y (downward) straight to mm +y, so an unmirrored part would print as the chiral mirror of the drawn layout.
...
- **New bin parameter**: add it to `BinConfig` in `src/lib/types.ts`, add a default in `DEFAULT_CONFIG` in `src/store.ts`, update both the manifold and JSCAD fallback paths in `gridfinity.ts`, then add the UI control in the appropriate tab using `updateConfig({ ... })`.
...
- **New printer behavior**: update `PRINTER_PROFILES`, `checkBedFit()`, `computeAutoSplitLines()`, or `checkPieceFit()` in `src/lib/printers.ts`; remember auto split mode is derived in the store.
...
- **Do not bypass the store contract.** Tabs should read/write through `useAppStore()`, and auto split mode should keep `splitLines` effective through existing setters.
