...
function openDatabase(): Promise<IDBDatabase> {
  databasePromise ??= new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error ?? new Error('Unable to open geometry cache.'));
    request.onblocked = () => reject(new Error('Geometry cache upgrade was blocked.'));
  });
...
async function evictLeastRecentlyUsed(): Promise<void> {
  ...
  const records = await requestResult(store.index(LAST_ACCESS_INDEX).getAll()) as CachedGeometryRecord[];
